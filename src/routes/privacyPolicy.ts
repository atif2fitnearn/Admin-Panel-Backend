...
router.post("/privacy", async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    const newContent = new PrivacyPolicyModel({ content });
...
router.get("/privacy", async (req: Request, res: Response): Promise<any> => {
  try {
    const content = await PrivacyPolicyModel.findOne().sort({ createdAt: -1 }); // Fetch the most recent content
    if (!content) {
...
router.put("/privacy", async (req, res): Promise<any> => {
  ...
  try {
    ...
    const existingContent = await PrivacyPolicyModel.findOne();
    if (existingContent) {
